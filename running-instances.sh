...
display_region_options() {
    echo "Select the AWS region to find running EC2 instances:"
    echo "1) us-east-1"
    echo "2) us-east-2"
    echo "3) us-west-1"
    echo "4) us-west-2"
...
echo "Finding running EC2 instances in region $region..."

running_instances=$(aws ec2 describe-instances --filters Name=instance-state-name,Values=running --query "Reservations[*].Instances[*].[InstanceId,InstanceType,State.Name,LaunchTime]" --output table --region "$region" 2>&1)

if [[ $? -ne 0 ]]; then
    echo "Error: $running_instances"
else
