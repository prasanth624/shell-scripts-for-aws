...
unattached_volumes=$(aws ec2 describe-volumes --filters Name=status,Values=available --query "Volumes[*].[VolumeId,Size,State,CreateTime]" --output table --region "$region" 2>&1)

if [[ $? -ne 0 ]]; then
    echo "Error: $unattached_volumes"
else
